...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: "/bin/bash /vagrant/install_docker_tools.sh"
  ...
  config.vm.define "manager" do |swarm|
    ...
    swarm.vm.provision "shell" do |s| 
      s.inline ="/bin/bash /vagrant/setup_swarm.sh manager" 
      #s.args = ["manager"]
    end
  ...
  config.vm.define "worker-01" do |swarm1|
    ...
    swarm1.vm.provision "shell" do |s1| 
      s1.inline ="/bin/bash /vagrant/setup_swarm.sh worker-01"
      #s1.args = ["worker-01"]
    end
  ...
  config.vm.define "worker-02" do |swarm2|
    ...
    swarm2.vm.provision "shell" do |s2| 
      s2.inline ="/bin/bash /vagrant/setup_swarm.sh worker-02"
      #s2.args = ["worker-02"]
    end
